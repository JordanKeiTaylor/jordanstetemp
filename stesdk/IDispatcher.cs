...
namespace Shared
{
    ...
    {
        void OnAddComponent<C>(Action<AddComponentOp<C>> callback)
            where C : IComponentMetaclass;

        void OnAddEntity(Action<AddEntityOp> callback);

        void OnAuthorityChange<C>(Action<AuthorityChangeOp> callback)
            where C : IComponentMetaclass;

        void OnCommandRequest<C>(Action<CommandRequestOp<C>> callback)
            where C : ICommandMetaclass, new();

        void OnCommandResponse<C>(Action<CommandResponseOp<C>> callback)
            where C : ICommandMetaclass, new();

        void OnComponentUpdate<C>(Action<ComponentUpdateOp<C>> callback)
            where C : IComponentMetaclass;
        ...
        void OnRemoveComponent<C>(Action<RemoveComponentOp> callback)
            where C : IComponentMetaclass;
